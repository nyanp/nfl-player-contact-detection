...
def pad_for_batch(list_tf_targets, list_tf_sources):
    ...
    return tf_targets, tf_sources, tf_pad_bools, num_points
...
def p2p_registration_features(tracking, helmets, meta, view_some_results=False):
    S = time.time()
    outputs = []
    for game_play in tracking["game_play"].unique():
        for view in ["Sideline", "Endzone"]:
            ...
            for frame, _df in helm_merged.groupby("frame"):
                ...
                player_ids += [_df["nfl_player_id"].values]
            ...
            tf_targets, tf_sources, tf_pad_bools, num_points = pad_for_batch(list_targets, list_sources)
            tf_flip_targets, tf_sources, tf_pad_bools, num_points = pad_for_batch(list_flip_target, list_sources)
            # print(num_points)
            ...
            list_targets, list_sources, list_residual = depad_for_batch(
                [view_target, trans_sources, residual_points], num_points)  # リストで好きなだけ渡せるようにするか。残渣も各ポイントでほしいし。
...
def add_p2p_matching_features(df, regist_df):
    regist_df["x_rel_position_offset_on_img"] = regist_df["x_position_offset_on_img"] / (regist_df["average_box_size"] + 1e-7)
    regist_df["y_rel_position_offset_on_img"] = regist_df["y_position_offset_on_img"] / (regist_df["average_box_size"] + 1e-7)
    ...
    df = pd.merge(df, regist_df.loc[regist_df["view"] == "Endzone", merge_columns], how="left",
                  ...
                  right_on=["game_play", "step", "nfl_player_id"]).rename(columns={"x_position_offset_on_img": "x_position_offset_on_img_End",
                                                                                   ...
                                                                                   "y_rel_position_offset_on_img": "y_rel_position_offset_on_img_End", }).drop(columns=["nfl_player_id"])
    print(df.shape)
    df.head()
    return df
