...
def p2p_registration_features(tracking, helmets, meta, view_some_results=False, num_possible_assign=2):
    ...
    for game_play in tracking["game_play"].unique():
        for view in ["Sideline", "Endzone"]:
            ...
            df_out[[f"p2p_registration_possible_player_{i}" for i in range(num_possible_assign)]] = np.concatenate(possible_player_ids, axis=0)
            df_out[[f"p2p_registration_possible_residual_{i}" for i in range(num_possible_assign)]] = np.concatenate(possible_residuals, axis=0)
