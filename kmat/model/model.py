...
from tensorflow.keras.layers import (Activation, Add, AveragePooling2D,
                                     ...
                                     Conv2D, Conv2DTranspose, Conv3D, Dense,
...
def build_model_explicit_distance_shift(input_shape=(256,256,3),
    ...
    if output_single_contact:
        # batch, num_player
        player_contact_single = Lambda(lambda x: tf.math.reduce_max(x[...,0] * x[...,1], axis=[2,3]), name="player_contact_single")(player_contact_w_mask)
    """
    ...
    losses = {"output_contact_label": bce_loss,
              "output_contact_label_player": bce_loss,
              "output_contact_label_total": bce_loss,
              "contact_map": l2_regularization,
              }
    loss_weights = {"output_contact_label": 1.,
                    "output_contact_label_player": 1.,
                    "output_contact_label_total":1,
                    "contact_map": 0.01,
                    }
    metrics = {"output_contact_label": [matthews_correlation_best],
    ...
    if output_single_contact:
        outputs += [player_contact_single]
        losses["player_contact_single"] = bce_loss
        loss_weights["player_contact_single"] = 0.25
        metrics["player_contact_single"] = [matthews_correlation_03]


    model = Model(inputs, outputs)
    if output_single_contact:
        sub_model = Model(inputs, [pairs_contact, output_contact_label_total, player_contact_single])
    else:
        sub_model = Model(inputs, [pairs_contact, output_contact_label_total])
    return model, sub_model, losses, loss_weights, metrics
