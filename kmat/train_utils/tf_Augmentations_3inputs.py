...
def resampler(img, warp_coords):
    warped = tfa.image.resampler(img[tf.newaxis], warp_coords[:,:,::-1])[0]#突貫で汚いため注意。
    return warped
...
class Rotation():
    ...
    def reset_params(self):
        self.rot = self.max_angle * tf.random.uniform([], -1, 1, dtype=tf.float32)
    ...
    def apply(self, img):
        ...
        #height = tf.minimum(height, width)
        #width=height
        # LIST DESTINATION PIXEL INDICES
        #x = tf.repeat(tf.range(width//2,-width//2,-1), height)
        #y = tf.tile( tf.range(-height//2,height//2),[width])
        x = tf.repeat(tf.range(height//2,-height//2,-1), width)# height axis
        y = tf.tile( tf.range(-width//2,width//2),[height])# width axis
        ...
        #idx2 = tf.clip_by_value(idx2[:2], #clip later
        #                        tf.stack([[-height//2+1],[-width//2+1]]),
        #                        tf.stack([[height//2],[width//2]]))
        
        # FIND ORIGIN PIXEL VALUES           
        ##idx3 = tf.stack( [height//2-idx2[0,], width//2-1+idx2[1,]] )
        idx3 = tf.stack( [tf.cast(height, tf.float32)/2-idx2[0,], 
                          tf.cast(width, tf.float32)/2-1+idx2[1,]] )
        
        idx3 = tf.clip_by_value(idx3, 
                                tf.stack([[0.],[0.]]),
                                tf.stack([[tf.cast(height, tf.float32)-1.],
                                          [ tf.cast(width, tf.float32)-1.]]))
        
        #idx3 = tf.stack([height//2-1+idx2[1,], width//2-idx2[0,]])
        #warped = tf.gather_nd(img, tf.transpose(idx3))
        warp_coords = tf.reshape(tf.transpose(idx3), [1,height*width, 2])
        warped = resampler(img, warp_coords)#tfa.image.resampler(img[tf.newaxis], warp_coords[:,:,::-1])[0]#突貫で汚いため注意。
        
        return tf.reshape(warped,[height,width,img_shape[2]])          
    ...
    def apply_to_point(self, inputs):# numpoints, 2(x,y)
        ...
        inputs = tf.clip_by_value(inputs, tf.zeros_like(self.height_width), self.height_width)
        return inputs[:,::-1]
