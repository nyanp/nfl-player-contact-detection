# -*- coding: utf-8 -*-
"""
Created on Wed Nov 24 21:16:20 2021
...
#現状のモーションネットはシーケンス入力なので、単画像でおこなうAugmentationはcrop-resize-dropぐらい？
...
class Compose():
    ...
    def __call__(self, inputs):#, targets):
        rgb_concat = tf.concat([inputs["rgb_1"], inputs["rgb_2"]], axis=0)
        for i, transform in enumerate(self.transforms):
            if tf.random.uniform(()) > transform.p:
                continue
            transform.reset_params()
            rgb_concat = transform(rgb_concat, "rgb")
        rgb_r, rgb_l = tf.split(rgb_concat, 2, axis=0)
        inputs["rgb_1"] = rgb_r
        inputs["rgb_2"] = rgb_l
        return inputs
...
class Resize():
    """
    """
    def __init__(self, height, width, target_height=None, target_width=None):
        self.p = 1.0
        self.height = height
        self.width = width
    ...
    def __call__(self, inputs, key):
        ...
        if key in ["rgb"]:
            if self.height is not None:
                inputs = self.apply(inputs, self.height, self.width, "bicubic")#bilinear
        if key in ["depth"]:
            if self.height is not None:
...
class __PertialBrightnessContrast():
...
class CoarseDropout():
    ...
    def __init__(self, p=0.5, 
        ...
        if not 0 < self.min_width <= self.max_width:
            raise ValueError("Invalid combination of min_width and max_width. Got: {}".format([min_width, max_width]))
        if apply_to_depth==False and apply_to_rgb==False:
            raise ValueError("set either or both [apply_to_depth, apply_to_rgb] = True")


    def __call__(self, img, key):
        if  key in ["rgb"] and self.apply_to_rgb:
            img = self.apply(img)
        if  key in ["depth"] and self.apply_to_depth:
            img = self.apply(img)
        return img
