...
def load_data(path,
    ...
    rgb_files = sorted([f for f in glob.glob(os.path.join(path, "*.jpg")) if "depth" not in f])[::frame_interval]
    ...
    game_id = int(os.path.basename(path)[:5])
    ...
    for i, [rgb_file, ann_file, pos_file, flow_12, flow_21] in enumerate(zip(rgb_files, annotation_files, pos_files, flow_files_12, flow_files_21)):
        ...
        if os.path.exists(pos_file):
            ...
            player_positions = np.ones((num_player, 2), np.float32)# dummy
            pos_npy_exists = False
...
def pad_box_and_labels_if_necessary(data, max_box_num=20, max_pair_num=40, player_pos_exist=True):
    ...
    data["player_id"] = tf.concat([data["player_id"], tf.tile(data["player_id"], [num_tile])], axis=0)[:max_box_num]
    if player_pos_exist:
...
def assign_input_output(data, num_cls):
    if LOAD_FLOW:
    ...
    outputs = {
              ...
              "pred_game_id": data["game_id"],
              # "pred_game_id": tf.cast(tf.gather(tf.eye(num_cls), data["game_id"]), tf.float32),
              }
...
def assign_input_output_w_info(data, player_pos_exist=False, 
                               info_keys=["num_labels", "num_player", "contact_pairlabels", "player_id"]):
...
def get_tf_dataset(list_dataset, 
    ...
    dataset = dataset.map(lambda x: assign_input_output(x, num_cls), num_parallel_calls=AUTO)
...
def get_tf_dataset_inference(list_dataset, 
    ...
    if padding:
        dataset = dataset.map(lambda x: pad_box_and_labels_if_necessary(x, max_box_num=max_box_num, max_pair_num=max_pair_num), num_parallel_calls=AUTO)
    dataset = dataset.map(lambda x: assign_input_output_w_info(x, player_pos_exist=True,
                                                               info_keys=["num_labels", "num_player", "contact_pairlabels", "player_id", "file"]), num_parallel_calls=AUTO)
