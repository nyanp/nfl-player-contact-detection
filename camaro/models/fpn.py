...
from .rois import transform_rois
...
class YoloxExt(nn.Module):
    def __init__(self, model_cfg):
        ...
        self.ext_g_linear = nn.Sequential(
            nn.Linear(10, self.num_channels),
            nn.LeakyReLU(negative_slope=0.2),
        ...
        self.ext_inter_linear = nn.Sequential(
            nn.Linear(10 * 2, self.num_channels),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Linear(self.num_channels, self.num_channels),
            nn.LeakyReLU(negative_slope=0.2),
        )
    ...
    def forward(self, inputs):
        ...
        end_coords = inputs['Endzone_coords'].squeeze(1)
        end_coords_dist = torch.cdist(end_coords, end_coords)  # b, n, n

        side_coords = inputs['Sideline_coords'].squeeze(1)
