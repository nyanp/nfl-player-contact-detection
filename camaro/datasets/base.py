...
from .video_transforms import Resize, Compose, ColorJitter, Resize
from .common import setup_df, normalize_img, to_torch_tensor, load_cv2_image, load_pickle, load_pil_image
...
class TrainDataset(Dataset):
    def __init__(self, cfg, fold, mode="train"):
        ...
        self.unique_ids_dict = load_pickle(f'../input/{cfg.unique_ids_dict_name}_{mode}_fold{fold}.pkl')
        self.inter_contact_dict = load_pickle(f'../input/{cfg.inter_contact_dict_name}_{mode}_fold{fold}.pkl')
        self.ground_contact_dict = load_pickle(f'../input/{cfg.ground_contact_dict_name}_{mode}_fold{fold}.pkl')
        self.helmet_dict = load_pickle(f'../input/{cfg.helmet_dict_name}_{mode}_fold{fold}.pkl')
        self.track_dict = load_pickle(f'../input/{cfg.track_dict_name}_{mode}_fold{fold}.pkl')
        ...
        assert self.duration % 2 == 1, 'duration has to be odd number.'
        self.half_duration = (self.duration - 1) // 2

        height, width = cfg.image_size
        self.resizer = Resize((height, width))
    ...
    def _get_item(self, game_play, frame):
        ...
        inputs['track'] = get_track_feats(track_feats, unique_ids)
